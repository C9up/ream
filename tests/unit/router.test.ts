...
describe('router > fluent chaining', () => {
  ...
  it('chains version and deprecation', () => {
    ...
    router
      .get('/orders', async () => {})
      .version('1')
      .deprecates('1', { sunset: '2027-01-01' })
...
describe('middleware > pipeline', () => {
  ...
  it('inline middleware executes between named and handler', async () => {
    const registry = new MiddlewareRegistry()
    const log: string[] = []
    registry.register('named', async (_ctx, next) => {
      log.push('named')
      await next()
    })
    const inlineMw = async (_ctx: HttpContext, next: () => Promise<void>) => {
      log.push('inline')
      await next()
    }
    const chain = registry.buildChain(['named'], [inlineMw], async () => {
      log.push('handler')
    })
...
describe('router > catch-all wildcard', () => {
  it('/assets/* matches /assets/css/main.css', () => {
    const router = new Router()
...
describe('router > makeUrl unreplaced params', () => {
  it('throws on missing required param', () => {
    const router = new Router()
    router.get('/users/:id/posts/:postId', async () => {}).as('user.post')
    expect(() => router.makeUrl('user.post', { id: '1' })).toThrow(/missing params.*:postId/)
  })
})
