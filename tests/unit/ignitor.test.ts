...
class MockHyperServer implements HyperServerLike {
  ...
  async request(method: string, path: string, body = '', headers: Record<string, string> = {}) {
    ...
    return await this.handler({ method, path, query: '', headers, body })
  }
}
...
describe('ignitor > 4-phase lifecycle (AdonisJS-style)', () => {
  ...
  it('reamrc manifest loads providers and preloads', async () => {
    ...
    const rc: ReamrcConfig = {
      ...
      preloads: [
        async () => {
          log.push('preload:routes')
        },
        async () => {
          log.push('preload:kernel')
        },
      ],
    }
  ...
  it('emits app:ready through the bus once all providers are ready', async () => {
    const { factory } = mockFactory()
    ...
    await new Ignitor({ port: 3000, serverFactory: factory })
      ...
      .start()
    expect(received).toEqual([{ environment: 'web' }])
  })
})
    ...
    await new Ignitor({ serverFactory: factory })
      ...
      .routes((r) =>
        r.get('/o/:id', async (ctx) => {
          ctx.response.send(ctx.params.id ?? '')
        }),
      )
      .start()
...
describe('ignitor > toolkit mode', () => {
  it('kernel without server', async () => {
    const app = await new Ignitor()
      .routes((r) =>
        r.get('/data', async (ctx) => {
          ctx.response.send('toolkit')
        }),
      )
      .start()
    const res = await app.getKernel()({
      method: 'GET',
      path: '/data',
      query: '',
      headers: {},
...
describe('ignitor > environment', () => {
  it('httpServer sets web', () => {
    expect(new Ignitor().httpServer().getEnvironment()).toBe('web')
  })
  it('testMode sets test', () => {
    expect(new Ignitor().testMode().getEnvironment()).toBe('test')
  })
